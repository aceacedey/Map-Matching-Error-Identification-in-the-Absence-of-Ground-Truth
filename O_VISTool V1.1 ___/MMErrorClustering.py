...
##      ax.plot(lon,lat,c=clr,marker='*', linestyle=' ')
   ...
   mmdf.apply(lambda x: ax.plot(x.edgeGeom.xy[0].tolist(),x.edgeGeom.xy[1].tolist(),c='g', linestyle='-',marker='.',linewidth=3),axis=1)
...
def FindDisconnectedSegments(d9): ## mg is the mapmatched graph
#if 1 == 1:
   df=d9[['u','v','edge','edgeGeom']].drop_duplicates(subset = 'edge', keep = 'first')
   mg = nx.from_pandas_edgelist(df, source='u', target='v', edge_attr='edge', create_using=nx.MultiGraph())
...
##   
   ...
   disconEdges = FindDisconnectedSegments(d9)
